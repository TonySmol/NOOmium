...
В вашем браузере (IndexedDB). Публичные копии расходятся по релеям Nostr — удалить релей из «мира» нельзя (это устройство протокола). Личные не покидают устройство в открытом виде никогда.
...
**— Чем это отличается от ленты в Telegram/X?**
...
Релиз «Большая чистка» — по итогам полного аудита кода (68 находок закрыто):

- 🔐 **Ключ в сейфе:** опциональное шифрованное хранилище ключа (ncryptsec + пароль)
- 🛡 **Верификация подписей** всех входящих событий — поддельные записи отбрасываются
- 🐛 Исправлено: редкая потеря правок при фоновой векторизации, кнопка «Копировать» при первом запуске, восстановление связи после долгого офлайна
- ⚡ Лента стала заметно легче на больших объёмах (кэш родословных, точечные обновления вместо полной пересборки)
- ⌨️ Доступность: клавиатурная навигация карточек, фокус-ловушки в окнах, поддержка reduced-motion
- 🧹 Вычищено ~300 строк мёртвого кода
